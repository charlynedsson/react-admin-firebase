...
export const recursivelyMapStorageUrls = async (
  ...
  if (isArray) {
    return Promise.all(
      (fieldValue as any[]).map(async (value, index) => {
        (fieldValue as any[])[index] = await recursivelyMapStorageUrls(
          fireWrapper,
          value
        );
      })
  ...
  if (isObject) {
    return Promise.all(
      Object.keys(fieldValue).map(async (key: string) => {
        const value = fieldValue[key];
        (fieldValue as any)[key] = await recursivelyMapStorageUrls(
          fireWrapper,
          value
        );
      })
