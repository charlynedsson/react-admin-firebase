...
export async function GetList<T extends ra.Record>(
  ...
  let filteredData = filterArray(softDeleted, filterSafe) as T[];
  if (params.pagination != null) {
    const pageStart = (params.pagination.page - 1) * params.pagination.perPage;
    const pageEnd = pageStart + params.pagination.perPage;
    filteredData = filteredData.slice(pageStart, pageEnd) as T[];
  }
  const total = filteredData.length;

  if (options.relativeFilePaths) {
    const fetchedData = await Promise.all(
      filteredData.map((doc) => recursivelyMapStorageUrls(fireWrapper, doc))
    );
    return {
      data: fetchedData,
      total,
